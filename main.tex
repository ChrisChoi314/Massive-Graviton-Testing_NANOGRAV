...
\title{Effects of the pulsar term in pulsar timing constraints on massive gravity}
...
The recent evidence of a stochastic gravitational wave background allows for investigations to find deviations from general relativity (GR), motivated by several observational phenomena that suggest that our current theory of gravity is incomplete. We explore the imprints of an extension of GR known as massive gravity on the propagation of gravitational waves: their modified dispersion relation and additional polarization modes. These effects distort the PTA overlap reduction function (ORF) expected from GR, known as the Hellings-Downs correlation. We correct earlier analytical results of the overlap reduction function for the vector and scalar modes, regularize their divergent behavior in the small anglular seperation and massless graviton limit via the suppression of their corresponding energy densities. With this robust treatment of the effective overlap reduction function, we perform Bayesian analysis on observational data from the NANOGrav 15-year dataset, and find that massive gravity favors HD with a bayes factor of $X$, with a frequentist statistic, built from a weighted sum of interpulsar correlations, yielding $p = Y$. 
\end{abstract}

\maketitle

\section{Introduction}\label{sec:intro}
The standard cosmological concordance model assumes that general relativity (GR) is the correct theory of gravity on cosmological length and time scales, and that the acceleration of the universe's expansion is due to a cosmological constant that becomes dominant at late times \cite{Dodelson:2020bqr}.
...
One of the most active areas of research in gravity theory is massive gravity, which stems from the assumption that the graviton has a non-zero mass $m_g$. It may seem at first like a strange assumption, but there is no reason why the mass of the graviton must be zero. The idea of a nonzero graviton mass has a long history, starting from the formulation of massive gravity at the linear level by Fierz and Pauli \cite{Fierz:1939ix} in the 1930s. In contrast to GR, where gravitational waves possess just two degrees of freedom with helicity (spin) $\pm 2$, generic massive gravity has an additional three degrees of freedom, namely the helicity  $\pm 1$ vector and $0$ scalar modes. As a result, massive gravity is subject to the van Dam-Veltman-Zakharov (vDVZ) discontinuity \cite{vanDam:1970vg,Zakharov:1970cc}, where when we try to take the massless limit, the five modes reduce to the two tensor modes and a coupled scalar degree of freedom that does not vanish. Taking into account the nonlinear effects of the strong gravitational potential, the Vainshtein mechanism \cite{Vainshtein:1972sx} is able to do away with the superfluous degree of freedom, freeing MG from the vDVZ discontinuity\footnote{GR is recovered in a strong gravitational field \cite{Tasinato:2013rza}, allowing us to verify GR in terrestrial and solar system level tests.}.

The effects of massive gravity appear on cosmological scales, possibly leading to an accelerated expansion \cite{DAmico:2011eto}. However, extensions in the nonlinear regime are unsatisfactory due to the presence of an unhealthy sixth ``ghost'' mode\footnote{Until 2010, it was thought that all Lorentz-invariant MG theories were characterized by the unhealthy presence of the ghost-mode, and thus were not valid \cite{deRham:2010kj}.} \cite{Boulware:1972yco}. More recently, groundbreaking progress has been made through the formulation of ghost-free massive gravity in the de Rham-Gabadadze-Tolley (dRGT) theory \cite{deRham:2010ik,deRham:2010kj}, and its generalization to bigravity \cite{Hassan:2011zd} (see Ref.~\cite{deRham:2023ngf} for a review). Since then, many different modifications of dRGT and bigravity have been proposed \cite{Hinterbichler:2011tt,deRham:2014zqa,Koyama:2015vza,deRham:2016nuf,Hinterbichler:2016try, Cusin:2016ytz, Kenna-Allison:2019tbu, Kazempour:2022giy,Hogas:2021saw,Hogas:2021lns,Hogas:2021fmr,Hogas:2019ywm,Gialamas:2025ciw,Gialamas:2023fly,Gialamas:2023lxj,Gialamas:2023aim,Dwivedi:2024okk},  
...
such as in the context of the creation of gravitational waves and their propagation \cite{DeFelice:2013awa,Gumrukcuoglu:2013nza,DeFelice:2013bxa,DeFelice:2015moy,Babichev:2015xha,Sakstein:2017bws}. 
The direct detection of gravitational waves at high frequencies by interferometers \cite{LIGOScientific:2016aoc,LIGOScientific:2016sjg, KAGRA:2020agh, VIRGO:2014yos} and the more recent evidence of the stochastic gravitational wave background (SGWB) at low frequencies, through various PTA collaborations \cite{Agazie:2023, Xu:2023wog,EPTA:2023sfo,EPTA:2023akd,EPTA:2023fyk, Zic:2023gta,Reardon:2023gzh}, have ignited further interest in gravitational wave astronomy, giving us the unique possibility of constraining fundamental physics and understanding the nature of gravity.
In our study, we use data collected by NANOGrav in their 15-year observation \cite{Agazie:2023} to help us better understand gravity and connect massive gravity to observables. 

In this paper, we derive the overlap reduction function (ORF) and the dispersion relation in ghost-free massive gravity, taking into account the effect of these extra polarization modes.
\red{In particular, we study the effect of the finite distance effect in the ORFs, which will be relevant when we leave in the pulsar terms in the integral.}

This paper is organized as follows: in Sec.\ \ref{sec:formalism}, we introduce ghost-free massive gravity, what overlap reductions emerge from this model, and the phenomenology of the finite-distance effect. In Sec.\ \ref{sec:results}, we use Bayesian analysis to constrain the values of the graviton mass based on the NANOGrav 15-year dataset. In Sec.\ \ref{sec:conclusion}, we discuss our findings and how we can conduct analysis on the energy density for future studies.  Throughout this paper, we use natural units, setting $c = $$\ \hbar = $$\ k_B = $$\ 1$, and use the + -- -- -- signature.

\section{Formalism}\label{sec:formalism} 
\subsection{Ghost-free massive gravity}\label{subsec:ghost_free} The following effects of MG on GWs have been used to place constraints on the graviton mass \cite{deRham:2016nuf}:

(i) \textit{Yukawa exponential suppression}---Yukawa suppression of the gravitational potential is expected when the force carrier possesses a nonzero mass \cite{Will:1997bb}. In the case of MG, suppression is of the form $\Phi \propto e^{-m_gR}/R$ \cite{Will:1997bb}, dependent on the length scale $R$. There is also a suppression of GWs at wavelengths larger than the Compton length scale of the graviton ($R_g \simeq m_g^{-1}$)\footnote{The length scale determined by the graviton mass through the Yukawa suppression can naively be treated as a ``gravitational causal horizon'', outside of which we expect the correlations of gravitational perturbations to be suppressed \cite{Will:1997bb}.}.
...
(iii) \textit{additional degrees of polarization}---These are characterized by the $\pm 1$ and $0$ modes. Certain theories of MG, such as the minimal theory of MG \cite{DeFelice:2015hla}, only have two tensor modes, as in GR, but a general theory of MG with Lorentz invariance has five in total \cite{Comelli:2013tja}.

A resounding confirmation of the graviton's non-zero mass will involve the observation of all three effects at the corresponding scales. In this paper, we turn our attention to the latter two effects, (ii) and (iii). Whenever the region of observation is much smaller than the radius of curvature induced by the GW, we may express the metric perturbation $h_{\mu\nu}$ as a plane wave \cite{Isi:2018miq}
\begin{equation}\label{eqn:planewave}
    h_{\mu\nu}(x) = \frac{1}{(2\pi)^4}\int d\boldsymbol{k} \ e^{ikx} h_{\mu\nu}(k)  \ .
\end{equation}
Here, we have the 4-dimensional measure $d\boldsymbol{k} \equiv d^4 k 2\delta(|\boldsymbol{k}|^2 - |k_{\omega}|^2)/|\boldsymbol{k}|$ \cite{Isi:2018miq}, where $k_{\omega} \equiv \boldsymbol{k}(\omega)$ and is given by the dispersion relation. The dispersion relation in MG is given by the  following expression: 
\begin{equation}\label{eq:dispersion}
    \omega^2 = |\boldsymbol{k}|^2+ m_g^2 \ ,
\end{equation}
where $\omega \equiv k_0 = 2\pi f$, the frequency of the GW. Whether the source is of astrophysical or cosmological origin, we are well within the regime where the plane-wave approximation is valid.

The dispersion relation provides a lower bound on the frequency for a GW, given by the mass of the graviton $m_g$, provided that $k$ goes to 0. In other words, $\omega$ must be greater than or equal to $m_g$; a departure from the behavior of GWs in GR. In a certain sense, we have two scenarios, one where the mass of the graviton is rather close to the frequencies that we probe, for which $m_g /\omega$ is a non-negligable fraction of 1, and one in which we are well within the GR limit, where $m_g /\omega \approx 0$. 

\subsection{Overlap reduction function}Suppose that there is a massive GW propagating in an arbitrary spatial direction specified by the unit vector $\hat{\Omega} = (\sin\theta \cos\varphi,
                        \sin\theta \sin\varphi,
                        \cos\theta)$.
...
\end{equation}
Here, $\beta$ is the normalization factor introduced so that $\Gamma(|f|) = \frac{1}{2}$ for coaligned coincident pulsars to match the conventions of the Hellings-Downs (HD) correlation \cite{Romano:2023zhb}, $\Omega_{\text{gw}}(|f|)$ is the energy density of the GWs, and $\delta_{ii'}$ is the Kronecker delta that requires that the modes $i, i' \in \{0,\pm 1, \pm 2\}$ be the same. Details of this derivation can be found in Refs.~\cite{Anholm:2008wy, Liang:2021bct}, deriving from earlier work \cite{Detweiler:1979wn, Estabrook:1975jtn, Kaufmann:1970, Sazhin:1978myk}. A wealth of literature has been produced to analyze the behavior of $\Omega_{\text{gw}}(|f|)$ in MG in the context of PTAs \cite{Choi:2023tun, Wu:2023rib, Kenjale:2024rsc, He:2021bqm}, but we turn our attention to the last factor in the expression: $\Gamma(|f|)$, otherwise known as the ORF. 

We assume that there are two pulsars, located at distances $L_1$ and $L_2$ from the solar-system barycenter in directions $\hat{p}_1$ and $\hat{p}_2$. We assume pulsar 1 is situated along the $z$-axis and pulsar 2 is situated on the $x-z$ plane at an angle $\xi$ from pulsar 1. We define the ORF: 
...
\end{equation}
where $I = \{T,V,S\}$ for each polarization type and we have defined the receiving function $F^{(i)}_j(\hat{\bf \Omega})$ and the exponential factor $\mathcal{E}_j(f, \hat{\Omega})$ for the $j$-th pulsar as 
