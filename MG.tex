...
%and to explain dark matter as a massive spin-2 particle \cite{Blas:2024jyh}. 
...
\end{equation} 
where $m_{g}$ is the mass of the graviton and $h_{\mu\nu}$ is the metric perturbation defined by the metric tensor as follows: $g_{\mu\nu} = \eta_{\mu\nu} + h_{\mu\nu}$. In its original form, the Boulware-Deser ghost is present, so we adopt the ghost-free formulation of it. Additionally, we use the quadratic expansion because we are only interested in working at the linear level. In order to obtain the equations of motion, in the linear regime, we first take the Fourier transform of the metric perturbation 
...
%In anticipation of our analysis of pulsar-pulsar correlations, we assume there are two pulsars, located distances $L_1$ and $L_2$ from the solar system barycenter, and in directions $\hat{p}_1$ and $\hat{p}_2$. For convenience, we assume pulsar 1 is situated along the $z$-axis and pulsar 2 is an angle $\xi$ away and is parallel to the $x-z$ plane, which simplifies our calculations. 
%CC: I want to define these a bit later
...
\end{equation}
This is simply a seperable differential equation, and is integrated from the pulsar to the solar system barycenter to yield the redshift (which we express in the Fourier space)
...
\end{equation}
A few details of the derivation have been excluded in this discussion, however, more careful derivations of this can be found in Refs.\ \cite{Anholm:2008wy, Liang:2021bct} which ultimately derive from even earlier work \cite{Detweiler:1979wn, Estabrook:1975jtn, Kaufmann:1970}.
%CC: I've spent a couple hours looking into this derivation, and I don't think the headache is worth it for now. I might eventually work out the full derivation in the appendix at a later date, but I dont know if it is necessary if liang and anholm basically do it in their appendices 
%CC: i changed my mind, I do want to give the reader an intuition for where the redshift equation comes from, so i have done a general overview of the steps taken to derive it. 
