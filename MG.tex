...
\begin{equation}\label{eqn:z}
    z(t) = \frac{\nu_0 - \nu(t)}{\nu_0} \ .
\end{equation}
PTA collabarations actually measure the integral of $z(t)$, with a quantity known as the anomalous residual, but it is almost trivial to rewrite our equations in terms of the anomalous residual if we so desire, so we proceed in our analysis with the redshift. In anticipation of our analysis of pulsar-pulsar correlations, let us assume we have two pulsars, located distances $L_1$ and $L_2$ from the solar system barycenter, and in directions $\hat{p}_1$ and $\hat{p}_2$. For convenience, we assume pulsar 1 is situated along the $z$-axis and pulsar 2 is an angle $\xi$ away and is parallel to the $x-z$ plane, which simplifies our calculations. 

We first note that 
Following the results from Refs.\ \cite{Detweiler:1979wn, Estabrook:1975jtn, Kaufmann:1970, Anholm:2008wy, Liang:2021bct}, we can write the redshift as
\begin{equation}
    z(t, \hat{\Omega}) = \frac{1}{2}\frac{\hat{p}^i\hat{p}^j}{1 + }
\end{equation}

From this, we use a result obtained by Detweiler \
