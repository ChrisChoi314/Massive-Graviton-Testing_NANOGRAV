...
\begin{abstract}
There are various outstanding questions about our universe, including the accelerated expansion of the universe and the exact source and nature of the gravitational wave background. These questions have been analyzed in the context of general relativity, leading to the success of the Hellings-Downs curve as a description of the angular correlation of the background. We propose a theory of ghost-free massive gravity, which would address both of these questions, and use the NANOGrav 15-year data and the Chinese Pulsar Timing Array Data Release I to temper our constraints. In massive gravity, three additional modes of polarization arise, leading to a modification of the dispersion relation and the overlap reduction function. By considering the lower bound for graviton-mass constraints from the dispersion relation, we scrutinize the possibility of detection. We find that the predicted overlap reduction function for such a graviton mass is a better fit than the Hellings-Down. Future PTA observation campaigns are therefore able to detect these additional modes of polarization and can be effectively used to constrain the graviton mass.
...
In this paper, we derive the overlap reduction function (ORF) and the dispersion relation in the case of ghost-free massive gravity, taking into account the effect of these extra helicity modes. We carefully calculate, at first analytically and then numerically, the contribution each of the helicity modes has on the ORF. The propagation of the different modes will be shown to modify the extrema of the ORFs. The speed of propagation is in general mode- and mass-dependent, but in this paper we will ignore this different propogation speed, due to constraints yielding a gravitational wave propagation speed very close to c \cite{Blas:2016qmn}. This work differs from previous work \cite{Liang:2021bct, Anholm:2008wy, Arjona:2024cex, Lee:2013awh} because of the particular emphasis we place on the non-suppression of the exponential factors, which is only possible because we analyze the ORFs in the context of an optimistic PTA observation prospect. 
