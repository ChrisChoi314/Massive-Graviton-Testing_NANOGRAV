...
A resounding confirmation of the graviton's non-zero mass will involve the observation of all three of these effects at the corresponding scales. In this Letter, we shall turn our attention to the latter two effects, (ii) and (iii). In a Friedmann-
Robertson-Walker (FRW) universe, GWs are described by perturbations to the
%TK maybe on top or above? 
metric tensor, $g_{\mu\nu}$, defined by $g_{\mu \nu}=\eta_{\mu \nu}+h_{\mu \nu}$. %\cite{Gumrukcuoglu:2012wt}
%\begin{equation}\label{eqn:metric}
%    \begin{aligned}
%        g_{\mu \nu} d x^\mu d x^\nu &=N(t)^2 d t^2-a(t)^2\left[\Omega_{n j}\left(x^k\right)+h_{n j}\right] d x^i d x^j , \\ 
%        \Omega_{n j}\left(x^k\right)&=\eta_{n j}+\frac{K \eta_{n l} \eta_{j m} x^l x^m}{1-K \eta_{l m} x^l x^m} \ . 
%    \end{aligned}
%\end{equation}
...
%TK since we use Emir's eq. use his convention
%CC avoided using Emir's because it seemed to only apply to tensor-type perturbations in the FRW metric, restricted to only 2 tensor modes propagating. Instead, used the convention that follows our more general analysis in https://arxiv.org/pdf/0809.0701, which is what they used for deriving these ORFs in massive gravity
Here, we use the (+,--,--,--) convention for the Minkowski metric $\eta_{\mu\nu}$ and Greek indices $\mu,\nu$ to indicate a summation over all four spacetime coordinates. Whenever the region of observation is much smaller than the radius of curvature induced by the GW, we may express $h_{\mu\nu}$ as a plane wave \cite{Isi:2018miq}
...
To build intuition for this, we remind ourselves that an identical dispersion relation is obtained from the Proca equations \cite{Proca:1936fbw}, which describe the massive helicity (spin) $\pm$ 1 particles. A careful derivation shows how the mass term emerges \cite{Wang:2024kir} for the massive photon. It is therefore not unexpected that the dispersion relation for the massive graviton, in the context of a helicity $\pm 2$ field, takes on the same form. 
...
We find that for NANOGrav15, HD gives a fit of $\chi^2$/d.o.f.\ $\sim 1.71$ whereas $\tilde{\Gamma}_T(f_1)$ with $|\boldsymbol{k}|/k_0 = 0.5$ gives a fit of $\chi^2$/d.o.f.\ $\sim 1.02$. The effective ORF from MG is therefore a better overall fit, although it still suffers from not matching the data in the first half of the data.
...
We find that for CPTA DR1, HD gives a fit of $\chi^2$/d.o.f.\ $\sim 3.00$ whereas $\tilde{\Gamma}_T(f_0)$ with $|\boldsymbol{k}|/k_0 = 0.1$ gives a fit of $\chi^2$/d.o.f.\ $\sim 1.51$. The effective ORF from MG is a significantly better fit than HD. 
%TK make more clear sentence above
In fact, the effective ORF from MG does remarkably well in matching the data. We summarize our statistics in Table \ref{tbl:chi}.
\begin{table}[ht] 
\centering
